...
if TYPE_CHECKING:
    ...
    VLLM_FORK_ATTN_PREFIX_CHUNK_BUCKETS: str = "4,8,12"
    VLLM_FORK_ATTN_TARGET_CTA_WAVES: int = 2
    VLLM_FORK_ATTN_ADAPTIVE_SPLIT_MIN_TOKENS: int = 4096
    VLLM_FORK_ATTN_TAIL_TILE_N: int = 0
    VLLM_FORK_ATTN_ENABLE_FOREST: bool = True
    VLLM_FORK_ATTN_ENABLE_FOREST_CUDAGRAPH: bool = True
    VLLM_FORK_ATTN_FOREST_CTA_BUCKETS: str = "64,128,256,384,512"
    VLLM_FORK_ATTN_CUDAGRAPH_CAPTURE_BUCKETS: str = ""
    VLLM_FORK_ATTN_FOREST_MAX_SPLITS: int = 0
    VLLM_FORK_ATTN_FANOUT_SCHEDULING_ENABLED: bool = True
    VLLM_FORK_ATTN_FANOUT_ADMISSION_WINDOW: int = 16
    VLLM_FORK_ATTN_FANOUT_ADMISSION_MAX_BYPASSES: int = 32
    VLLM_FORK_ATTN_DP_PREFIX_ROUTING: bool = False
    VLLM_FORK_ATTN_DP_PREFIX_LOAD_SLACK: int = 32
    VLLM_FORK_ATTN_DP_PREFIX_WARM_TTL: float = 30.0
    VLLM_FORK_ATTN_DP_PREFIX_MIN_BLOCKS: int = 4
    VLLM_FORK_ATTN_DP_PREFIX_MAX_WARM_REQUESTS: int = 1024
    VLLM_FORK_ATTN_DP_GRAPH_SLACK_BUCKETS: int = 1
    VLLM_FORK_ATTN_DP_WORK_SLACK_TOKENS: int = 0
    VLLM_FORK_ATTN_DP_DECODE_TOKEN_WEIGHT: int = 16
    VLLM_FORK_ATTN_DP_PREFIX_REPLICATION_RELAX_RATIO: float = 0.20
    VLLM_FORK_ATTN_DP_ARRIVAL_WAVE_MS: float = 10.0
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    "VLLM_FORK_ATTN_TARGET_CTA_WAVES": lambda: int(
        os.getenv("VLLM_FORK_ATTN_TARGET_CTA_WAVES", "2")
    ),
    "VLLM_FORK_ATTN_ADAPTIVE_SPLIT_MIN_TOKENS": lambda: int(
        os.getenv("VLLM_FORK_ATTN_ADAPTIVE_SPLIT_MIN_TOKENS", "4096")
    ),
    "VLLM_FORK_ATTN_TAIL_TILE_N": lambda: int(
        os.getenv("VLLM_FORK_ATTN_TAIL_TILE_N", "0")
    ),
    ...
    "VLLM_FORK_ATTN_ENABLE_FOREST_CUDAGRAPH": lambda: bool(
        int(os.getenv("VLLM_FORK_ATTN_ENABLE_FOREST_CUDAGRAPH", "1"))
    ),
    "VLLM_FORK_ATTN_FOREST_CTA_BUCKETS": lambda: os.getenv(
        "VLLM_FORK_ATTN_FOREST_CTA_BUCKETS", "64,128,256,384,512"
    ),
    "VLLM_FORK_ATTN_CUDAGRAPH_CAPTURE_BUCKETS": lambda: os.getenv(
    ...
    "VLLM_FORK_ATTN_FANOUT_SCHEDULING_ENABLED": lambda: bool(
        int(os.getenv("VLLM_FORK_ATTN_FANOUT_SCHEDULING_ENABLED", "1"))
    ),
    ...
    "VLLM_FORK_ATTN_DP_PREFIX_ROUTING": lambda: bool(
        int(os.getenv("VLLM_FORK_ATTN_DP_PREFIX_ROUTING", "0"))
    ),
    "VLLM_FORK_ATTN_DP_PREFIX_LOAD_SLACK": lambda: int(
        os.getenv("VLLM_FORK_ATTN_DP_PREFIX_LOAD_SLACK", "32")
    ),
    "VLLM_FORK_ATTN_DP_PREFIX_WARM_TTL": lambda: float(
        os.getenv("VLLM_FORK_ATTN_DP_PREFIX_WARM_TTL", "30")
    ),
    "VLLM_FORK_ATTN_DP_PREFIX_MIN_BLOCKS": lambda: int(
        os.getenv("VLLM_FORK_ATTN_DP_PREFIX_MIN_BLOCKS", "4")
    ),
    "VLLM_FORK_ATTN_DP_PREFIX_MAX_WARM_REQUESTS": lambda: int(
        os.getenv("VLLM_FORK_ATTN_DP_PREFIX_MAX_WARM_REQUESTS", "1024")
    ),
    "VLLM_FORK_ATTN_DP_GRAPH_SLACK_BUCKETS": lambda: int(
        os.getenv("VLLM_FORK_ATTN_DP_GRAPH_SLACK_BUCKETS", "1")
    ),
    "VLLM_FORK_ATTN_DP_WORK_SLACK_TOKENS": lambda: int(
        os.getenv("VLLM_FORK_ATTN_DP_WORK_SLACK_TOKENS", "0")
    ),
    "VLLM_FORK_ATTN_DP_DECODE_TOKEN_WEIGHT": lambda: int(
        os.getenv("VLLM_FORK_ATTN_DP_DECODE_TOKEN_WEIGHT", "16")
    ),
    "VLLM_FORK_ATTN_DP_PREFIX_REPLICATION_RELAX_RATIO": lambda: float(
        os.getenv("VLLM_FORK_ATTN_DP_PREFIX_REPLICATION_RELAX_RATIO", "0.20")
    ),
    "VLLM_FORK_ATTN_DP_ARRIVAL_WAVE_MS": lambda: float(
        os.getenv("VLLM_FORK_ATTN_DP_ARRIVAL_WAVE_MS", "10")
    ),
