...
def sync_cudagraph_and_dp_padding(
    ...
    tensor[4][dp_rank] = fork_plan.capacity if fork_plan is not None else 0
    tensor[5][dp_rank] = fork_plan.max_splits if fork_plan is not None else 0
    ...
    fork_capacities_across_dp = tensor[4]
    fork_splits_across_dp = tensor[5]
    ...
    # Mixed ranks can use the common Flash graph captured alongside every Fork
    # graph. Keep the upstream token/query-length dispatch and padding rules.
    synced_fork_plan = (
        ForkGraphPlan(
