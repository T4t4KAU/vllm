...
struct Block {
  ...
  __device__ Block(const Params& params, const int bidb, const int bidh)
      : num_seqs(reinterpret_cast<int*>(params.num_seqs_per_CTA_ptr)[bidb]),
        sum_s_q(num_seqs * HRatio),
        kv_in_CTA(reinterpret_cast<int*>(params.kv_in_CTA_ptr)[bidb]),
        cta_rank(reinterpret_cast<int*>(params.CTA_rank_ptr)[bidb]),
  ...
  __forceinline__ __device__ thrust::pair<int, int> o_lse_offset(
    ...
    int lse_offset = q_head_offset * params.softmax_lseaccum_head_stride +
                     cta_rank * params.softmax_lseaccum_rank_stride;
...
inline __device__ void fork_kernel(const Params& params, const int bidb,
  ...
  if (binfo.num_seqs <= 0 || binfo.kv_in_CTA <= 0 ||
      binfo.num_seqs > params.query_table_width || binfo.sum_s_q > kBlockM ||
      binfo.cta_rank < 0 || binfo.cta_rank >= params.max_split_per_seq ||
      cute::ceil_div(binfo.kv_in_CTA, params.page_block_size) >
          params.block_table_width) {
    return;
  }
  ...
  const int row_offset_k = bidh * params.kv_head_stride;
  const int row_offset_v = bidh * params.v_head_stride;
...
inline __global__ void gather_kernel(Params params) {
  ...
  const int lse_offset = bidb * params.softmax_lse_batch_stride +
                         bidh * params.softmax_lse_head_stride;
  const int split_in_seq =
      reinterpret_cast<int*>(params.num_split_per_seq_ptr)[bidb];
  if (split_in_seq <= 0 || split_in_seq > params.max_split_per_seq) {
    return;
  }
  ...
  Tensor gOaccum = make_tensor(make_gmem_ptr(oaccum_ptr),
                               ...
                               make_stride(params.oaccum_rank_stride, _1{}));
  ...
  sLse[tid] =
      tid < split_in_seq
          ? softmax_lseaccum_ptr[tid * params.softmax_lseaccum_rank_stride]
          : -INFINITY;
  __syncthreads();
