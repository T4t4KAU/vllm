...
def test_fork_attention_rejects_strided_split_counts() -> None:
    device = torch.device("cuda")
    q = torch.ones(2, 1, 4, 64, dtype=torch.float16, device=device)
    cache = torch.ones(1, 16, 1, 64, dtype=q.dtype, device=device)
    counts = torch.ones(4, dtype=torch.int32, device=device)[::2]
    with pytest.raises(RuntimeError, match="num_split_per_seq.*contiguous"):
        ops.fork_attention(
            torch.empty_like(q),
            torch.empty(2, 4, 1, dtype=torch.float32, device=device),
