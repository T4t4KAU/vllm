...
def test_async_scheduling_pp_allows_rescheduling_with_output_placeholders(monkeypatch):
    ...
    # This CPU scheduler test does not launch workers on physical GPUs.
...
def test_adaptive_prefill_admits_cached_continuation(async_scheduling, enabled):
    """A long running prefill must leave compute for a cached Agent turn."""
    scheduler = create_scheduler(
        enable_prefix_caching=True,
        max_model_len=32768,
        async_scheduling=async_scheduling,
        additional_config={"agentrix_adaptive_prefill": enabled},
    )
    seed = create_requests(1, num_tokens=8192, max_tokens=1, req_ids=["seed"])[0]
    scheduler.add_request(seed)
    _model_output(scheduler, scheduler.schedule(), [[1000]])
    cold = _capacity_request("cold", 24576)
    scheduler.add_request(cold)
    first = scheduler.schedule()
    assert first.num_scheduled_tokens == {"cold": 8192}
    _model_output(scheduler, first, [[]])

    warm = create_requests(1, num_tokens=8448, req_ids=["warm"])[0]
    scheduler.add_request(warm)
    output = scheduler.schedule()
    if enabled:
        assert output.num_scheduled_tokens == {"cold": 1024, "warm": 256}
        assert warm.prefill_stats.num_local_cached_tokens == 8192
    else:
        assert output.num_scheduled_tokens == {"cold": 8192}


@pytest.mark.parametrize("async_scheduling", [False, True])
def test_adaptive_prefill_caps_total_work_while_decode_progresses(async_scheduling):
    """Several short prefills must share a budget, rather than each get a cap."""
    scheduler = create_scheduler(
        enable_prefix_caching=True,
        async_scheduling=async_scheduling,
        additional_config={"agentrix_adaptive_prefill": True},
    )
    decode = create_requests(1, num_tokens=16, req_ids=["decode"])[0]
    scheduler.add_request(decode)
    _model_output(scheduler, scheduler.schedule(), [[1000]])
    requests = create_requests(
        4, num_tokens=1024, req_ids=[f"new-{i}" for i in range(4)]
    )
    for request in requests:
        scheduler.add_request(request)
    output = scheduler.schedule()
    ...
    assert (
        sum(v for k, v in output.num_scheduled_tokens.items() if k != "decode") <= 2048
    )
    assert any(r.status == RequestStatus.WAITING for r in requests)


@pytest.mark.parametrize("async_scheduling", [False, True])
def test_adaptive_prefill_reaches_short_request_behind_cold_waiters(async_scheduling):
    scheduler = create_scheduler(
        enable_prefix_caching=True,
        max_model_len=32768,
        async_scheduling=async_scheduling,
    ...
    cold = [_capacity_request(f"cold-{i}", 24576) for i in range(3)]
    warm = create_requests(1, num_tokens=256, req_ids=["short"])[0]
    for request in [*cold, warm]:
        scheduler.add_request(request)
    output = scheduler.schedule()
    assert output.num_scheduled_tokens == {"cold-0": 1024, "short": 256}
    assert list(scheduler.skipped_waiting) == cold[1:]


@pytest.mark.parametrize("async_scheduling", [False, True])
def test_adaptive_prefill_bounds_deferrals_and_cleans_cancelled_requests(
    async_scheduling,
):
    scheduler = create_scheduler(
        enable_prefix_caching=True,
    ...
    for i in range(2):
        scheduler.add_request(_capacity_request(f"cold-{i}", 49152))
    for i in range(4):
        short = create_requests(1, num_tokens=16, max_tokens=1, req_ids=[f"short-{i}"])[
            0
        ]
        scheduler.add_request(short)
        output = scheduler.schedule()
        assert short.request_id in output.num_scheduled_tokens
        assert "cold-1" not in output.num_scheduled_tokens
        _model_output(scheduler, output, [[], [1000]])
    # Four budget deferrals trigger a normal FCFS step. Memory constraints
    # still apply; this is not a guarantee that every waiter is admitted.
    output = scheduler.schedule()
    assert output.num_scheduled_tokens == {"cold-0": 8192}
    assert not scheduler._adaptive_prefill_deferred
    _model_output(scheduler, output, [[]])
    scheduler.add_request(create_requests(1, num_tokens=16, req_ids=["last"])[0])
    scheduler.schedule()
    assert scheduler._adaptive_prefill_deferred == {"cold-1": 1}
    scheduler.finish_requests("cold-1", RequestStatus.FINISHED_ABORTED)
    assert not scheduler._adaptive_prefill_deferred


def test_adaptive_prefill_probe_does_not_change_cache_or_reserve_for_unfittable_turn():
    scheduler = create_scheduler(
        enable_prefix_caching=True,
        max_model_len=32768,
        additional_config={"agentrix_adaptive_prefill": True},
    ...
    free_order = list(pool.free_block_queue.get_all_free_blocks())
    stats = dataclasses.asdict(manager.prefix_cache_stats)
    assert scheduler._agent_prefill_budget() is not None
    assert [b.ref_cnt for b in pool.blocks] == refs
    assert list(pool.free_block_queue.get_all_free_blocks()) == free_order
    assert dataclasses.asdict(manager.prefix_cache_stats) == stats
    assert list(scheduler.waiting) == [warm]
    # A large cached prompt is cheap to compute, but it still needs KV space.
    scheduler.running.append(_capacity_request("occupied", 16384))
    manager.watermark_blocks = pool.get_num_free_blocks()
    assert scheduler._agent_prefill_budget() is None
    manager.watermark_blocks = 0
    # A cached block near the end cannot establish a hit across an earlier hole.
    root = pool.get_cached_block(warm.block_hashes[0], [0])[0]
    pool.evict_blocks({root.block_id})
    assert pool.get_cached_block(warm.block_hashes[463], [0]) is not None
    assert scheduler._agent_prefill_budget() is None


def test_adaptive_prefill_avoids_full_scans_of_obviously_long_prefills(monkeypatch):
    scheduler = create_scheduler(
        enable_prefix_caching=True,
        max_model_len=32768,
        additional_config={"agentrix_adaptive_prefill": True},
    ...
    scheduler.add_request(seed)
    _model_output(scheduler, scheduler.schedule(), [[1000]])
    for request in create_requests(8, num_tokens=12288, same_prompt=True):
        scheduler.add_request(request)
    scan = Mock(side_effect=AssertionError("Unnecessary full-prefix scan"))
    monkeypatch.setattr(
        scheduler.kv_cache_manager.coordinator, "find_longest_cache_hit", scan
    )
    assert scheduler._agent_prefill_budget() is None
    scan.assert_not_called()


def test_adaptive_prefill_preserves_async_kv_resume_without_extra_connector_lookups():
    # Exercise the connector protocol with the existing async-transfer fixture;
    # production opt-in only accepts the native OffloadingConnector.
    scheduler = _setup_remote_kv_resume(512, matched_tokens=256)
    scheduler.adaptive_prefill_enabled = True
    lookup = Mock(wraps=scheduler.connector.get_num_new_matched_tokens)
    scheduler.connector.get_num_new_matched_tokens = lookup
    scheduler._agent_prefill_budget()
    lookup.assert_not_called()
    output = scheduler.schedule()
    assert output.num_scheduled_tokens == {"r1": 1, "r2": 256}
    assert scheduler.requests["r2"].prefill_stats.num_external_cached_tokens == 256
    lookup.assert_not_called()


@pytest.mark.parametrize("async_scheduling", [False, True])
@pytest.mark.parametrize("transfer", ["ready", "pending", "failed"])
def test_adaptive_prefill_reserves_for_completed_load_behind_cold_prefill(
    async_scheduling, transfer, monkeypatch
):
    scheduler = create_scheduler(
        enable_prefix_caching=True,
        max_model_len=32768,
        async_scheduling=async_scheduling,
        use_kv_connector=mock_kv(8192, True),
    )
    lookup = scheduler.connector.get_num_new_matched_tokens
    monkeypatch.setattr(
        scheduler.connector,
        "get_num_new_matched_tokens",
        lambda request, computed: (0, False)
        if request.request_id == "cold"
        else lookup(request, computed),
    )
    scheduler.add_request(_capacity_request("cold", 24576))
    _model_output(scheduler, scheduler.schedule(), [[]])
    warm = create_requests(1, num_tokens=8448, req_ids=["warm"])[0]
    scheduler.add_request(warm)
    # Allow the load to start alongside an already running cold prefill.
    scheduler.scheduler_config.long_prefill_token_threshold = 1024
    output = scheduler.schedule()
    assert warm.status == RequestStatus.WAITING_FOR_REMOTE_KVS
    scheduler.update_from_output(
        output,
        ModelRunnerOutput(
            req_ids=["cold"],
            ...
            kv_connector_output=KVConnectorOutput(
                finished_recving={"warm"} if transfer != "pending" else set()
            ),
        ),
    )
    if transfer == "failed":
        scheduler.failed_recving_kv_req_ids.add("warm")
    scheduler.scheduler_config.long_prefill_token_threshold = 0
    scheduler.adaptive_prefill_enabled = True
    output = scheduler.schedule()
    assert output.num_scheduled_tokens == (
        {"cold": 1024, "warm": 256} if transfer == "ready" else {"cold": 8192}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"use_kv_connector": mock_kv(16, True)},
    ],
)
def test_adaptive_prefill_rejects_unsupported_configurations(kwargs):
    config: dict[str, Any] = dict(
        enable_prefix_caching=True,
        additional_config={"agentrix_adaptive_prefill": True},
    )
    config.update(kwargs)
    with pytest.raises(ValueError, match="Adaptive prefill requires"):
        create_scheduler(**config)


@pytest.mark.parametrize("async_scheduling", [False, True])
def test_gpu_session_retention_scheduler_lifecycle(async_scheduling):
    scheduler = create_scheduler(
        enable_prefix_caching=True,
        num_blocks=33,
        async_scheduling=async_scheduling,
    ...
    assert len(pool.session_retention.free) == 4
    assert pool.get_num_free_blocks() == 32
    assert pool.get_usage() == 0
    continuation = create_requests(1, num_tokens=64, max_tokens=1, req_ids=["next"])[0]
    continuation.session_id = "agent"
    scheduler.add_request(continuation)
    output = scheduler.schedule()
    assert output.num_scheduled_tokens["next"] == 16
    scheduler.update_from_output(
        output,
        ModelRunnerOutput(
            req_ids=["next"],
            req_id_to_index={"next": 0},
            sampled_token_ids=[[1000]],
            logprobs=None,
            prompt_logprobs_dict={},
            pooler_output=[],
    ...
    assert pool.get_num_free_blocks() == 32
    assert not pool.session_retention.free
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"additional_config": {"agentrix_gpu_session_retention": True}},
        {
            "additional_config": {
                "agentrix_gpu_session_retention": {"protected_fraction": 0.75}
            }
        },
    ],
)
def test_gpu_session_retention_rejects_unsupported_configuration(kwargs, monkeypatch):
    monkeypatch.setattr("vllm.config.parallel.current_platform.device_count", lambda: 2)
    config: dict[str, Any] = dict(
        enable_prefix_caching=True,
        additional_config={"agentrix_gpu_session_retention": {}},
    ...
    with pytest.raises(
        ValueError,
        match="GPU session retention|agentrix_gpu_session|protected_fraction",
    ):
        create_scheduler(**config)
