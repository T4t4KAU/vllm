...
def _gpu_session_blocks(pool, name, session_id, token_start=0, count=4):
    request = make_request(
        name,
        list(range(token_start, token_start + count * pool.hash_block_size)),
        pool.hash_block_size,
        sha256,
    )
    request.session_id = session_id
    if pool.session_retention is not None:
        pool.session_retention.on_request(request)
    blocks = pool.get_new_blocks(count)
    pool.cache_full_blocks(request, blocks, 0, count, pool.hash_block_size, 0)
    return request, blocks


def _finish_gpu_session(pool, request, blocks):
    request.status = RequestStatus.FINISHED_STOPPED
    if pool.session_retention is not None:
        pool.session_retention.on_finished(request, blocks)
    pool.free_blocks(reversed(blocks))


@pytest.mark.parametrize("enabled", [False, True])
def test_gpu_session_retention_preserves_prefix_across_cold_traffic(enabled):
    """A returning Agent retains its prefix despite newer one-off requests."""
    pool = BlockPool(9, True, 4)
    if enabled:
        pool.enable_session_retention(protected_fraction=0.5)
    request, blocks = _gpu_session_blocks(pool, "turn1", "agent")
    _finish_gpu_session(pool, request, blocks)
    cold, cold_blocks = _gpu_session_blocks(pool, "cold", None, token_start=100)
    _finish_gpu_session(pool, cold, cold_blocks)

    pool.get_new_blocks(4)
    hit = pool.get_cached_block(request.block_hashes[0], [0])
    assert bool(hit) is enabled
    assert pool.get_num_free_blocks() == 4
    assert pool.get_usage() == 0.5


def test_gpu_session_retention_never_reduces_allocatable_capacity():
    pool = BlockPool(9, True, 4)
    pool.enable_session_retention(protected_fraction=0.5)
    request, blocks = _gpu_session_blocks(pool, "turn1", "agent")
    _finish_gpu_session(pool, request, blocks)
    ...
    assert pool.get_cached_block(request.block_hashes[0], [0]) is not None
    allocated = pool.get_new_blocks(8)
    assert len({b.block_id for b in allocated}) == 8
    assert all(b.ref_cnt == 1 for b in allocated)
    assert pool.get_num_free_blocks() == 0
    assert not pool.session_retention.free


def test_gpu_session_expiry_uses_empty_blocks_before_evicting_cache(monkeypatch):
    clock = [100.0]
    monkeypatch.setattr("vllm.v1.core.block_pool.monotonic", lambda: clock[0])
    pool = BlockPool(9, True, 4)
    pool.enable_session_retention(protected_fraction=0.5)
    request, blocks = _gpu_session_blocks(pool, "turn1", "agent")
    _finish_gpu_session(pool, request, blocks)
    ...
    assert {b.block_id for b in allocated}.isdisjoint(b.block_id for b in blocks)
    assert pool.get_cached_block(request.block_hashes[0], [0]) == [blocks[0]]


@pytest.mark.parametrize("release", ["expiry", "budget"])
def test_gpu_session_release_evicts_tail_before_shared_prefix(monkeypatch, release):
    clock = [100.0]
    monkeypatch.setattr("vllm.v1.core.block_pool.monotonic", lambda: clock[0])
    pool = BlockPool(9, True, 4)
    pool.enable_session_retention(protected_fraction=0.5)
    # Logical prefix order must survive recycling non-consecutive physical IDs.
    recycled = pool.get_new_blocks(4)
    pool.free_blocks([recycled[i] for i in (2, 0, 3, 1)])
    request, blocks = _gpu_session_blocks(pool, "turn1", "agent")
    _finish_gpu_session(pool, request, blocks)
    if release == "expiry":
        pool.get_new_blocks(4)  # Consume the remaining empty blocks.
        clock[0] += 16
    else:
        newer, newer_blocks = _gpu_session_blocks(pool, "new", "other", 100)
        _finish_gpu_session(pool, newer, newer_blocks)
    assert pool.get_new_blocks(1) == [blocks[-1]]
    assert pool.get_cached_block(request.block_hashes[0], [0]) == [blocks[0]]


def test_gpu_session_retention_shared_prefix_respects_references(monkeypatch):
    clock = [100.0]
    monkeypatch.setattr("vllm.v1.core.block_pool.monotonic", lambda: clock[0])
    pool = BlockPool(9, True, 4)
    pool.enable_session_retention(protected_fraction=0.5)
    request, blocks = _gpu_session_blocks(pool, "parent", "agent")
    pool.touch(blocks)  # A child still owns the same physical prefix.
    _finish_gpu_session(pool, request, blocks)
    ...
    assert pool.get_num_free_blocks() == 4
    clock[0] += 16
    pool.get_new_blocks(4)  # Expiration cannot return referenced blocks.
    assert all(b.ref_cnt == 1 and b.block_hash is not None for b in blocks)
    assert pool.get_num_free_blocks() == 0
    pool.free_blocks(reversed(blocks))
    assert pool.get_num_free_blocks() == 4


def test_gpu_session_retention_bounds_metadata_and_handles_invalidation():
    pool = BlockPool(9, True, 4)
    pool.enable_session_retention(protected_fraction=0.5, max_sessions=2)
    for i in range(20):
        request, blocks = _gpu_session_blocks(pool, str(i), str(i), token_start=i * 20)
        _finish_gpu_session(pool, request, blocks)
        retention = pool.session_retention
        assert len(retention.sessions) <= 2
        assert len(retention.leases) <= 4
        assert len(retention.expirations) <= 8
        assert len(retention.free) <= 4
    pool.evict_blocks({b.block_id for b in blocks})
    assert not retention.free
    assert pool.get_num_free_blocks() == 8
    assert pool.reset_prefix_cache()
    assert not retention.leases and not retention.expirations and not retention.sessions
    assert len(pool.get_new_blocks(8)) == 8


def test_gpu_session_retention_learns_only_observed_inter_turn_gaps(monkeypatch):
    clock = [100.0]
    monkeypatch.setattr("vllm.v1.core.block_pool.monotonic", lambda: clock[0])
    pool = BlockPool(9, True, 4)
    pool.enable_session_retention(protected_fraction=0.5)
    ...
    _finish_gpu_session(pool, request, blocks)
    clock[0] += 1
    request2, blocks2 = _gpu_session_blocks(pool, "turn2", "agent", token_start=100)
    _finish_gpu_session(pool, request2, blocks2)
    # Initial 7.5s gap estimate blends with the observed 1s gap, then doubles.
    assert {lease[0] for lease in pool.session_retention.leases.values()} == {109.5}


@pytest.mark.parametrize(
    "status", [RequestStatus.PREEMPTED, RequestStatus.FINISHED_ABORTED]
)
def test_gpu_session_retention_does_not_lease_failed_turns(status):
    pool = BlockPool(9, True, 4)
    pool.enable_session_retention(protected_fraction=0.5)
    request, blocks = _gpu_session_blocks(pool, "turn1", "agent")
    ...
    pool.session_retention.on_finished(request, blocks)
    pool.free_blocks(reversed(blocks))
    assert not pool.session_retention.leases
    assert not pool.session_retention.free


def test_gpu_session_retention_preserves_pool_invariants_under_churn(monkeypatch):
    """Reuse, expire, invalidate and reclaim leases without losing/doubling blocks."""
    rng = random.Random(20260707)
    clock = [100.0]
    monkeypatch.setattr("vllm.v1.core.block_pool.monotonic", lambda: clock[0])
    pool = BlockPool(33, True, 4)
    pool.enable_session_retention(max_sessions=4)
    live = []
    for i in range(1000):
        clock[0] += rng.random()
        action = rng.randrange(4)
        if action == 0 and pool.get_num_free_blocks():
            live.append(
                _gpu_session_blocks(
                    pool,
                    str(i),
                    str(i % 7),
                    i * 40,
                    min(rng.randint(1, 8), pool.get_num_free_blocks()),
                )
            )
        elif action == 1 and live:
            req, blocks = live.pop(rng.randrange(len(live)))
            _finish_gpu_session(pool, req, blocks)
        elif action == 2:
            cached = [b for b in pool.blocks if b.block_hash is not None]
            if cached:
                block = rng.choice(cached)
                pool.touch([block])
                pool.free_blocks([block])
        else:
            pool.evict_blocks({rng.randrange(1, 33)})
        free_ids = [b.block_id for b in pool.free_block_queue.get_all_free_blocks()]
        free_ids.extend(pool.session_retention.free)
        assert len(free_ids) == len(set(free_ids)) == pool.get_num_free_blocks()
        expected_refs = [0] * 33
        for _, blocks in live:
            for block in blocks:
                expected_refs[block.block_id] += 1
        assert [b.ref_cnt for b in pool.blocks[1:]] == expected_refs[1:]
        assert set(free_ids) == {i for i in range(1, 33) if expected_refs[i] == 0}
    for req, blocks in live:
        _finish_gpu_session(pool, req, blocks)
    assert pool.reset_prefix_cache()
    assert len(pool.get_new_blocks(32)) == 32
