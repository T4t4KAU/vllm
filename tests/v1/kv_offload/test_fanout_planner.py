...
def test_select_prioritizes_shared_work_position_fanout_and_age() -> None:
    planner = FanoutChunkPlanner()
    blocks = [
        make_block(0, request_id="hot", fanout=8),
        make_block(1, request_id="hot", fanout=8),
    ...
    assert [chunk.request_id for chunk in plan.chunks] == ["hot"]
...
def test_min_fanout_filters_private_suffix_chunks() -> None:
    ...
    assert [(chunk.request_id, chunk.fanout) for chunk in chunks] == [
        ("shared", 8)
    ]
